/**
 * DevolucionesClientesTab.jsx
 * ───────────────────────────
 * Pestaña de devoluciones de clientes: listado, registro y detalle.
 * - Al seleccionar una venta, carga sus productos y muestra el cliente.
 * - Parcial: permite elegir productos y cantidades a devolver.
 * - Total: todos los productos se incluyen automáticamente.
 * - El detalle muestra la tabla de productos devueltos.
 */
...
import {
  ...
  AlertCircle, PackageX, CheckCircle, XCircle, Minus,
} from 'lucide-react';
...
const CATEGORIA_MOTIVO_OPTIONS = [
  ...
  { value: 'cambio_cliente', label: 'Cambio solicitado por cliente' },
...
const MOTIVO_LABELS = {
  ...
  cambio_cliente: 'Cambio de cliente', vencido: 'Producto vencido',
  danado: 'Producto dañado', otro: 'Otro',
};

export const DevolucionesClientesTab = ({ negocioId }) => {
  ...
  /* Estado de productos del detalle de la venta */
  const [detalleVenta, setDetalleVenta] = useState([]);
  const [loadingDetalle, setLoadingDetalle] = useState(false);
  /* Productos seleccionados para devolver: { [detalleVentaId]: cantidadADevolver } */
  ...
  } = useDevoluciones(negocioId, 'clientes');

  /* ─── Ventas del negocio (para seleccionar al crear) ─── */
  const { data: ventas = [] } = useQuery({
    queryKey: ['ventas', negocioId],
    queryFn: () => ventasService.getByNegocio(negocioId),
    enabled: !!negocioId && isFormOpen,
    select: (data) => data.filter((v) =>
      v.estado === 'completada' || v.estado === 'parcialmente_pagada'
    ),
  });
  ...
  /* ─── Cargar detalle de venta cuando cambia la selección ─── */
  const cargarDetalleVenta = useCallback(async (ventaId) => {
    if (!ventaId) { setDetalleVenta([]); setProductosSeleccionados({}); return; }
    setLoadingDetalle(true);
    try {
      const items = await ventasService.getDetalle(ventaId);
      setDetalleVenta(items);
      /* Si es total, seleccionar todo automáticamente */
      if (tipoDevolucion === 'total') {
        const sel = {};
        items.forEach((item) => { sel[item.id] = Number(item.cantidad); });
        setProductosSeleccionados(sel);
      } else {
        setProductosSeleccionados({});
      }
    } catch { setDetalleVenta([]); }
    setLoadingDetalle(false);
  }, [tipoDevolucion]);

  useEffect(() => { cargarDetalleVenta(selectedVentaId); }, [selectedVentaId]);

  /* Cuando cambia el tipo a total, seleccionar todos; a parcial, limpiar */
  useEffect(() => {
    if (!detalleVenta.length) return;
    if (tipoDevolucion === 'total') {
      const sel = {};
      detalleVenta.forEach((item) => { sel[item.id] = Number(item.cantidad); });
      setProductosSeleccionados(sel);
    } else {
      setProductosSeleccionados({});
    }
  }, [tipoDevolucion, detalleVenta]);

  /* Venta seleccionada (objeto) */
  const selectedVenta = useMemo(
    () => ventas.find((v) => String(v.id) === String(selectedVentaId)),
    [ventas, selectedVentaId],
  );

  /* ─── Helpers de selección de productos ─── */
  const toggleProducto = (detalleId, maxCantidad) => {
  ...
  /* Calcular total de la devolución según productos seleccionados */
  const calcularTotalDevolucion = useCallback(() => {
    let total = 0;
    for (const [detalleId, qty] of Object.entries(productosSeleccionados)) {
      const item = detalleVenta.find((d) => String(d.id) === String(detalleId));
  ...
  const filtered = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      result = result.filter((d) =>
        ...
        (d.cliente?.nombreCompleto || '').toLowerCase().includes(q)
      );
    }
    return result;
  ...
  const handleCreate = async (formData) => {
    ...
    const devolucionData = {
      ...
      sede: { id: sede?.id || selectedVenta?.sede?.id || selectedVenta?.sedeId },
      venta: selectedVentaId ? { id: Number(selectedVentaId) } : null,
      cliente: selectedVenta?.cliente ? { id: selectedVenta.cliente.id } : null,
    ...
    /* 2) Crear cada detalle de devolución */
    if (devolucionId) {
      const promises = selectedItems.map(([detalleId, qty]) => {
        const item = detalleVenta.find((d) => String(d.id) === String(detalleId));
        if (!item) return null;
        return detalleDevolucionesService.create({
          devolucion: { id: devolucionId },
          producto: { id: item.productoId },
          detalleVenta: { id: item.id },
          cantidad: qty,
          precioUnitario: item.precioUnitario,
          total: Number(item.precioUnitario) * qty,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-gray-900">Devoluciones de Clientes</h2>
          <p className="text-sm text-gray-500 mt-1">Gestiona las devoluciones asociadas a ventas realizadas a clientes</p>
        </div>
      ...
      <Card>
        <div className="flex flex-col sm:flex-row gap-3 p-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Buscar por número, motivo, cliente..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
      ...
      <Card>
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-gray-400"><RotateCcw size={40} className="mb-3" /><p className="font-medium">No se encontraron devoluciones de clientes</p><p className="text-sm mt-1">Las devoluciones registradas aparecerán aquí</p></div>
        ) : (
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title="Registrar Devolución de Cliente" size="xl">
        <form onSubmit={handleSubmit(handleCreate)} className="space-y-4">
          {/* Seleccionar venta */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Venta Asociada <span className="text-red-500">*</span></label>
            <select value={selectedVentaId} onChange={(e) => setSelectedVentaId(e.target.value)}
          ...
          {/* Nombre del cliente auto-detectado */}
          {selectedVenta && (
          ...
          {selectedVentaId && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Productos a devolver {tipoDevolucion === 'total' ? '(todos incluidos)' : <span className="text-red-500">*</span>}
              </label>
              {loadingDetalle ? (
                <div className="flex items-center justify-center py-4"><Loader2 size={20} className="animate-spin text-green-600" /><span className="ml-2 text-sm text-gray-500">Cargando productos...</span></div>
              ) : detalleVenta.length === 0 ? (
                <p className="text-sm text-gray-400 py-2">No se encontraron productos en esta venta</p>
              ) : (
                  ...
                  <table className="w-full text-sm min-w-[600px]">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-center">Cant. Comprada</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {detalleVenta.map((item) => {
                        const maxQty = Number(item.cantidad);
                        const isSelected = !!productosSeleccionados[item.id];
                        const qty = productosSeleccionados[item.id] || 0;
                        ...
                        return (
                          <tr key={item.id} className={isSelected ? 'bg-green-50' : ''}>
                            {!isTotal && (
                              <td className="px-3 py-2 text-center">
                                <input type="checkbox" checked={isSelected}
                                  onChange={() => toggleProducto(item.id, maxQty)}
                                  className="h-4 w-4 text-green-600 border-gray-300 rounded focus:ring-green-500" />
                              </td>
                            )}
                            <td className="px-3 py-2 font-medium">{item.nombreProducto || `Producto #${item.productoId}`}</td>
                            <td className="px-3 py-2 text-center text-gray-500">{maxQty}</td>
                            <td className="px-3 py-2">
                              {(isSelected || isTotal) ? (
                                <div className="flex items-center justify-center gap-1">
                                  <button type="button" onClick={() => updateCantidad(item.id, qty - 1, maxQty)}
                                  ...
                                  <input type="number" min={1} max={maxQty} value={qty}
                                    onChange={(e) => updateCantidad(item.id, e.target.value, maxQty)}
                                    disabled={isTotal}
                                    className="w-14 text-center border border-gray-300 rounded px-1 py-0.5 text-sm focus:ring-1 focus:ring-green-500 outline-none" />
                                  <button type="button" onClick={() => updateCantidad(item.id, qty + 1, maxQty)}
                            ...
                            <td className="px-3 py-2 text-right font-medium">
                              {(isSelected || isTotal) ? formatCurrency(Number(item.precioUnitario) * qty) : '—'}
                            </td>
                          </tr>
                        );
                      })}
                    ...
                    <tfoot className="bg-gray-50 font-medium">
                      <tr>
                        <td colSpan={tipoDevolucion === 'parcial' ? 5 : 4} className="px-3 py-2 text-right">Total a devolver:</td>
                        <td className="px-3 py-2 text-right text-green-700 text-base">{formatCurrency(calcularTotalDevolucion())}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
              )}
              {tipoDevolucion === 'parcial' && Object.keys(productosSeleccionados).length === 0 && detalleVenta.length > 0 && (
                <p className="text-xs text-amber-600 mt-1">Seleccione al menos un producto para devolver</p>
              )}
            </div>
          )}
          ...
          <div>
            ...
            <Textarea {...register('detalleMotivo')} placeholder="Describa detalladamente el motivo de la devolución..." rows={3} />
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            ...
            <Button type="submit" disabled={isCreating || !selectedVentaId || Object.keys(productosSeleccionados).length === 0}>
      ...
      {/* ═══════════════════════════════════════════════════════════
         ...
         ═══════════════════════════════════════════════════════════ */}
      <Modal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} title="Detalle de Devolución" size="xl">
        {selected && (
          <div className="space-y-4">
            ...
            <div>
              <p className="text-xs text-gray-500 uppercase tracking-wider mb-2">Productos en devolución</p>
              {loadingDetalleDevolucion ? (
                <div className="flex items-center justify-center py-4"><Loader2 size={18} className="animate-spin text-green-600" /><span className="ml-2 text-sm text-gray-500">Cargando productos...</span></div>
                ...
                <div className="border border-gray-200 rounded-lg overflow-x-auto">
                  <table className="w-full text-sm min-w-[600px]">
                    ...
                    <tfoot className="bg-gray-50 font-medium">
                      <tr>
                        <td colSpan={4} className="px-3 py-2 text-right">Total devolución:</td>
                        <td className="px-3 py-2 text-right text-green-700">{formatCurrency(detalleDevolucionItems.reduce((s, d) => s + Number(d.total || 0), 0))}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  </table>
